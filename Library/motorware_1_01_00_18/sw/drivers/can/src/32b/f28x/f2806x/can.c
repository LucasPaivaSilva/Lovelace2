...
#include "sw/drivers/fifo/src/fifo.h"
...
uint32_t reverse_bytes(uint32_t value) {
    return ((value >> 24) & 0xFF) |         // Move byte 3 para byte 0
           ((value << 8) & 0xFF0000) |      // Move byte 1 para byte 2
           ((value >> 8) & 0xFF00) |        // Move byte 2 para byte 1
           ((value << 24) & 0xFF000000);    // Move byte 0 para byte 3
}

uint16_t convert_to_0_1_units(float_t value) {
    return (uint16_t)(value * 10.0);
}

// Função para enviar a mensagem CAN com o ID 0x046
void send_CAN_message_046(ECAN_Handle handle, float_t hv_voltage, float_t motor_current, float_t motor_temp, float_t controller_temp) {
    // Converte os valores de float_t para uint16_t no formato 0.1 unidades por bit
    uint16_t hv_voltage_int = convert_to_0_1_units(hv_voltage);
    uint16_t motor_current_int = convert_to_0_1_units(motor_current);
    uint16_t motor_temp_int = convert_to_0_1_units(motor_temp);
    uint16_t controller_temp_int = convert_to_0_1_units(controller_temp);

    // Prepara os dados para enviar na mensagem CAN
    uint32_t MDL_t = ((uint32_t)(hv_voltage_int & 0xFFFF)) | (((uint32_t)(motor_current_int & 0xFFFF)) << 16);
    uint32_t MDH_t = ((uint32_t)(motor_temp_int & 0xFFFF)) | (((uint32_t)(controller_temp_int & 0xFFFF)) << 16);

    MDL_t = reverse_bytes(MDL_t);
    MDH_t = reverse_bytes(MDH_t);

    // Envia a mensagem CAN utilizando a função ECAN_putDataMailbox_ID
    ECAN_putDataMailbox_ID(handle, MailBox0, 0x300, DLC_8, MDL_t, MDH_t);
    ECAN_transmitMsg(handle, MailBox0);
}

// Função para enviar a mensagem CAN com o ID 0x047
void send_CAN_message_047(ECAN_Handle handle, float_t motor_torque, float_t motor_power_out, float_t motor_power_in, float_t motor_rpm) {
    // Converte apenas o torque para 0.1Bit/unidade
    uint16_t motor_torque_int = convert_to_0_1_units(motor_torque);

    // Resto é  1bit/unidade
    uint16_t motor_power_out_int = (uint16_t)(motor_power_out);
    uint16_t motor_power_in_int = (uint16_t)(motor_power_in);
    uint16_t motor_rpm_int = (uint16_t)(motor_rpm);

    // Prepara os dados para enviar na mensagem CAN
    uint32_t MDL_t = ((uint32_t)(motor_torque_int & 0xFFFF)) | (((uint32_t)(motor_power_out_int & 0xFFFF)) << 16);
    uint32_t MDH_t = ((uint32_t)(motor_power_in_int & 0xFFFF)) | (((uint32_t)(motor_rpm_int & 0xFFFF)) << 16);

    MDL_t = reverse_bytes(MDL_t);
    MDH_t = reverse_bytes(MDH_t);

    // Envia a mensagem CAN utilizando a função ECAN_putDataMailbox_ID
    ECAN_putDataMailbox_ID(handle, MailBox1, 0x301, DLC_8, MDL_t, MDH_t);
    ECAN_transmitMsg(handle, MailBox1);
}

// Função para enviar a mensagem CAN com o ID 0x048
void send_CAN_message_048(ECAN_Handle handle, uint8_t pwm_data, uint8_t dsp_usage, uint8_t torque_code, uint8_t error_code) {
    // Prepara os dados para enviar na mensagem CAN
    uint32_t MDL_t = (uint32_t)(pwm_data & 0xFF) |
                         ((uint32_t)(dsp_usage & 0xFF) << 8) |
                         ((uint32_t)(torque_code & 0xFF) << 16) |
                         ((uint32_t)(error_code & 0xFF) << 24);
    uint32_t MDH_t = 0; // Os outros bytes podem ser usados para outros dados ou deixados como 0

    // Reverter os bytes antes de enviar
    MDL_t = reverse_bytes(MDL_t);
    MDH_t = reverse_bytes(MDH_t);

    // Envia a mensagem CAN utilizando a função ECAN_putDataMailbox_ID
    ECAN_putDataMailbox_ID(handle, MailBox2, 0x302, DLC_8, MDL_t, MDH_t);
    ECAN_transmitMsg(handle, MailBox2);
}

// Função para enviar a mensagem CAN com o ID 0x049
void send_CAN_message_049(ECAN_Handle handle, float value1, float value2) {
    // Buffer para armazenar os dados da mensagem CAN
    uint8_t data[8] = {0};

    // Copiar os valores float_t para o buffer de dados
    memcpy(&data[0], &value1, sizeof(float));
    memcpy(&data[4], &value2, sizeof(float));

    // Preparar os dados para enviar na mensagem CAN, ajustando a ordem dos bytes para little-endian
    uint32_t MDL_t = ((uint32_t)data[0]) | ((uint32_t)data[1] << 8) | ((uint32_t)data[2] << 16) | ((uint32_t)data[3] << 24);
    uint32_t MDH_t = ((uint32_t)data[4]) | ((uint32_t)data[5] << 8) | ((uint32_t)data[6] << 16) | ((uint32_t)data[7] << 24);

    // Envia a mensagem CAN utilizando a função ECAN_putDataMailbox_ID
    ECAN_putDataMailbox_ID(handle, MailBox3, 0x049, DLC_8, MDL_t, MDH_t);
    ECAN_transmitMsg(handle, MailBox3);
}

// Função para enviar a mensagem CAN com o ID 0x050
void send_CAN_message_050(ECAN_Handle handle, float_t value1, float_t value2) {
    // Buffer para armazenar os dados da mensagem CAN
    uint8_t data[8] = {0};

    // Copiar os valores float_t para o buffer de dados
    memcpy(&data[0], &value1, sizeof(float_t));
    memcpy(&data[4], &value2, sizeof(float_t));

    // Preparar os dados para enviar na mensagem CAN, ajustando a ordem dos bytes para little-endian
    uint32_t MDL_t = ((uint32_t)data[0]) | ((uint32_t)data[1] << 8) | ((uint32_t)data[2] << 16) | ((uint32_t)data[3] << 24);
    uint32_t MDH_t = ((uint32_t)data[4]) | ((uint32_t)data[5] << 8) | ((uint32_t)data[6] << 16) | ((uint32_t)data[7] << 24);
    // Envia a mensagem CAN utilizando a função ECAN_putDataMailbox_ID
    ECAN_putDataMailbox_ID(handle, MailBox4, 0x050, DLC_8, MDL_t, MDH_t);
    ECAN_transmitMsg(handle, MailBox4);
}


// Função para enviar a mensagem CAN com o ID 0x051
void send_CAN_message_051(ECAN_Handle handle, float_t value1, float_t value2) {
    // Buffer para armazenar os dados da mensagem CAN
    uint8_t data[8] = {0};
    ...
    // Envia a mensagem CAN utilizando a função ECAN_putDataMailbox_ID
    ECAN_putDataMailbox_ID(handle, MailBox5, 0x051, DLC_8, MDL_t, MDH_t);
    ECAN_transmitMsg(handle, MailBox5);
}

// Função para enviar a mensagem CAN com o ID 0x052
void send_CAN_message_052(ECAN_Handle handle, float_t value1, float_t value2) {
    // Buffer para armazenar os dados da mensagem CAN
    uint8_t data[8] = {0};
    ...
    // Envia a mensagem CAN utilizando a função ECAN_putDataMailbox_ID
    ECAN_putDataMailbox_ID(handle, MailBox6, 0x052, DLC_8, MDL_t, MDH_t);
    ECAN_transmitMsg(handle, MailBox6);
}


void send_CAN_floats(ECAN_Handle handle, ECAN_MailBox_e mailbox, uint32_t can_id, float_t value1, float_t value2)
{
    // Copiar os valores float para variáveis de 32 bits
    uint32_t bin_value1, bin_value2;

    memcpy(&bin_value1, &value1, sizeof(float));
    memcpy(&bin_value2, &value2, sizeof(float));

    // Monta MDL e MDH no padrão big-endian (byte mais significativo no byte 0)
    uint32_t MDL_t = ((bin_value1 >> 24) & 0xFF)        |  // Byte 0
                     ((bin_value1 >> 8)  & 0xFF00)      |  // Byte 1
                     ((bin_value1 << 8)  & 0xFF0000)    |  // Byte 2
                     ((bin_value1 << 24) & 0xFF000000);    // Byte 3

    uint32_t MDH_t = ((bin_value2 >> 24) & 0xFF)        |  // Byte 4
                     ((bin_value2 >> 8)  & 0xFF00)      |  // Byte 5
                     ((bin_value2 << 8)  & 0xFF0000)    |  // Byte 6
                     ((bin_value2 << 24) & 0xFF000000);    // Byte 7

    // Envia a mensagem CAN utilizando a função ECAN_putDataMailbox_ID
    ECAN_putDataMailbox_ID(handle, mailbox, can_id, DLC_8, MDL_t, MDH_t);

    // Solicita a transmissão da mensagem
    ECAN_transmitMsg(handle, mailbox);
}

void send_CAN_byte(ECAN_Handle handle, ECAN_MailBox_e mailbox, uint32_t can_id, uint8_t value)
{
    // Garante que valor está no intervalo 0-127
    if (value > 127) value = 127;

    // Monta MDL com o valor no byte mais significativo (bits 31:24)
    uint32_t MDL_t = ((uint32_t)value) << 24;
    uint32_t MDH_t = 0;
